...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        SImg::tic();
        if (lambda > 1e-9){
            sl_richardson_lucy_tv_2d(buffer_in_padding, buffer_psf, buffer_out, sx_out, sy_out, niter, lambda);
        }
        else{
            sl_richardson_lucy_2d(buffer_in_padding, buffer_psf, buffer_out, sx_out, sy_out, niter);
        }
        SImg::toc();
