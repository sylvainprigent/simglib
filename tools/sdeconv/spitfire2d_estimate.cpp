...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        for (int pow_lambda = 1; pow_lambda <= 20; pow_lambda++)
        {
            ...
            std::string outputImageFile = outputImageDir + "/lambda_" + std::to_string(lambda) + ".tif";
        ...
        STable* table = new STable(lambdas.size(), 3);
        table->setHeader(0, "lambda");
        table->setHeader(1, "energy");
        table->setHeader(2, "tv");
        for (int i = 0 ; i < lambdas.size() ; i++)
        {
            table->set(i, 0, lambdas[i]);
            table->set(i, 1, energies[i]);
            table->set(i, 2, tvs[i]);
        }  

        SCSV scsv;
        scsv.set(table);
        scsv.write("energies.csv");
