...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        bool verbose = true;
        if (verbose){
            observer->message("ZeroPadding: input image: " + inputImageFile);
            observer->message("ZeroPadding: output image: " + outputImageFile);
            observer->message("ZeroPadding: padx: " + std::to_string(padx));
            observer->message("ZeroPadding: pady: " + std::to_string(pady));
            observer->message("ZeroPadding: padz: " + std::to_string(padz));
        }
        ...
        float* buffer_out = new float[sx_out*sy_out*sy_out];
        ...
        int ctrl = SImg::hanning_padding_3d(buffer_in, buffer_out, sx, sy, sz, sx_out, sy_out, sz_out);
        if (ctrl > 0){
