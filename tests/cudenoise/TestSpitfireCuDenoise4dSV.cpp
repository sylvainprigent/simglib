...
int main(int argc, char *argv[])
{
    ...
    SImg::cuda_spitfire4d_denoise(noisy_image, sx, sy, sz, st, denoised_image, regularization, weighting, deltaz, deltat, niter, "SV", true, observable);

    SImg::toc();
    delete STimerAccess::instance();

    //SImageReader::write(new SImageFloat(denoised_image, sx, sy, sz, st), SAMPLE4DDENOISEDSV);
