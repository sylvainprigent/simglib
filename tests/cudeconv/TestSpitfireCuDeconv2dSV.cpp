...
int main(int argc, char *argv[])
{
    ...
    SImg::cuda_spitfire2d_deconv(blurry_image, sx, sy, psf, deconv_image, regularization, weighting, niter, "SV", true, observable);
    
    SImg::toc();
    delete STimerAccess::instance();

    //SImageReader::write(new SImageFloat(deconv_image, sx, sy), SAMPLE2DDECONVSV);

    // calculate error with the reference image
    std::cout << "calculate error:" << std::endl;
    SImageFloat *resultImage = dynamic_cast<SImageFloat *>(SImageReader::read(SAMPLE2DDECONVSV, 32));
