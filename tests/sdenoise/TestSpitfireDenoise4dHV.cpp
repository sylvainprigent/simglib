...
int main(int argc, char *argv[])
{
    ...
    std::cout << "ref image = " << SAMPLE4DDENOISED << std::endl;
    ...
    SImg::spitfire4d_hv(noisy_image_norm, sx, sy, sz, st, denoised_image, regularization, weighting, niter, deltaz, deltat, true, observable);
    ...
    //SImageReader::write(new SImageFloat(denoised_image, sx, sy, sz, st), "./bin/denoise4d/denoised_4d.txt");

    // calculate error with the reference image
    std::cout << "calculate error:" << std::endl;
    SImageFloat *resultImage = dynamic_cast<SImageFloat *>(SImageReader::read(SAMPLE4DDENOISED, 32));
