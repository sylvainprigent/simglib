...
void cuda_spitfire3d_denoise(float *blurry_image, unsigned int sx, unsigned int sy, unsigned int sz, float *psf, float *denoised_image, const float &regularization, const float &weighting, const unsigned int &niter, const float& delta, const std::string &method, bool verbose, SObservable *observable);
