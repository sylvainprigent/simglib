...
void cuda_spitfire2d_denoise(float *blurry_image, unsigned int sx, unsigned int sy, float *psf, float *denoised_image, const float &regularization, const float &weighting, const unsigned int &niter, const std::string &method, bool verbose, SObservable *observable);
