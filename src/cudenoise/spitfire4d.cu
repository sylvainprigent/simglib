...
__global__
void sv_4d_primal(unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float primal_step, float primal_weight, 
...
__global__
void hv_4d_primal(unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float primal_step, float primal_weight, float primal_weight_comp, float sqrt2, float *denoised_image, float *noisy_image,
...
__global__
void sv_4d_dual(unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float dual_weight, float dual_weight_comp, float*auxiliary_image, float* dual_images0, float* dual_images1,
...
__global__
void hv_4d_dual(unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float dual_weight, float dual_weight_comp, float sqrt2, float*auxiliary_image, float* dual_images0, float* dual_images1,
...
namespace SImg{
    ...
    void cuda_spitfire4d_denoise_sv(float* noisy_image, unsigned int sx, unsigned int sy, unsigned int sz, unsigned int sz, float* denoised_image, const float& regularization, const float& weighting, const unsigned int& niter, bool verbose, SObservable* observable)
    {
        ...
        for (int iter = 0; iter < niter; iter++) {
            ...
            sv_4d_primal<<<blockSize3d,gridSize3d>>>(sx, sy, sz, st, primal_step, primal_weight, primal_weight_comp, 
            ...
            // dual    
            sv_4d_dual<<<blockSize3d,gridSize3d>>>(sx, sy, sz, st, dual_weight, dual_weight_comp, 
    ...
    void cuda_spitfire4d_denoise_hv(float* noisy_image, unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float* denoised_image, const float& regularization, const float& weighting, const unsigned int& niter, bool verbose, SObservable* observable)
    {
        ...
        for (int iter = 0; iter < niter; ++iter) {
            ...
            hv_4d_primal<<<gridSize3d, blockSize3d>>>(sx, sy, sz, st, primal_step, primal_weight, primal_weight_comp, sqrt2, 
                                                     cu_denoised_image, cu_noisy_image, dual_images0, dual_images1, 
                                                     dual_images2, dual_images3, dual_images4, dual_images5, dual_images6,
                                                     dual_images7, dual_images8, dual_images9, dual_images10);
            ...
            // dual    
            hv_4d_dual<<<gridSize2d, blockSize2d>>>(sx, sy, sz,, st, dual_weight, dual_weight_comp, sqrt2, 
                                                   auxiliary_image, dual_images0, 
    ...
    void cuda_spitfire4d_denoise(float *blurry_image, unsigned int sx, unsigned int sy, unsigned int sz, unsigned int st, float *psf, float *denoised_image, const float &regularization, const float &weighting, const unsigned int &niter, const std::string &method, bool verbose, SObservable *observable)
    {
        ...
        normL2(blurry_image, sx, sy, sz, st, 1, blurry_image_norm);

        // run denoising
        if (method == "SV")
        {
            cuda_spitfire4d_denoise_sv(blurry_image_norm, sx, sy, sz, st, psf, deconv_image, regularization, weighting, niter, verbose, observable);
        }
        else if (method == "HV")
        {
            cuda_spitfire4d_denoise_hv(blurry_image_norm, sx, sy, sz, st, psf, deconv_image, regularization, weighting, niter, verbose, observable);
        }
        else
        {
